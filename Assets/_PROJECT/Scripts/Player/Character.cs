...
namespace ZFGinc.Player
{
    ...
    public class Character : MonoBehaviour
    {
        ...
        [Space(15f)]
        public bool canCameraRotation = true;
        ...
        {
            get => _maxSprintSpeed;
            set => _maxSprintSpeed = Mathf.Max(0.0f, value);
        }
        ...
        {
            if (!isSprinting && sprintInputPressed && CanSprint())
            {
                isSprinting = true;

                _cachedMaxWalkSpeed = maxWalkSpeed;
                maxWalkSpeed = _maxSprintSpeed;

            }
            else if (isSprinting && (!sprintInputPressed || !CanSprint()))
            {
                isSprinting = false;
        ...
        public virtual void RemovePhysicsVolume(Collider other)
        {
            if (other.TryGetComponent(out PhysicsVolume volume) && _physicsVolumes.Contains(volume))
        ...
        public virtual float GetMaxSpeed()
        {
            ...
            {
                case MovementMode.Walking:
                    return IsCrouched() ? maxWalkSpeedCrouched : (isSprinting ? _maxSprintSpeed : maxWalkSpeed);
        ...
        /// <summary>
        /// Request the Character to sprint. The request is processed on the next simulation update.
        /// Call this from an input event (such as a button 'down' event).
        /// </summary>
        ...
        /// <summary>
        /// Request the Character to end a sprint. The request is processed on the next simulation update.
        /// Call this from an input event (such as a button 'down' event).
        /// </summary>
        ...
        {
            if (!IsFalling()) jumpCurrentCount = 0;
